...
const KelompokPage = () => {
  ...
  const [fetchDataDesa, setFetchDataDesa] = useState<Option[]>([]);

  const navigate = useNavigate();

  const fetchData = async () => {
    try {
      return await fetchDesaKelompok({
        page,
        rows,
        filterInput,
        status,
  ...
  const {
    data: dataListKelompok,
    isFetching: isRefetchingKelompok,
    refetch: refetchListKelompok,
  } = useQuery({
    queryKey: ["dataListKelompok", page, rows],
    queryFn: fetchData,
    refetchOnWindowFocus: false,
  });

  const loadAllData = async () => {
    const [desa] = await Promise.all([
      fetchOptions("/api/v1/desa/all", "data_tempat_sambung", "nama_desa"),
    ]);

    setFetchDataDesa(desa);
  };
  ...
  const DetailDataFetch = async (Kode: any, visibilityOption: any) => {
    ...
    try {
      const response = await fetchDetailKelompok(Kode);
      ...
      const navigationState = {
        ...
        fetchdataDesa: fetchDataDesa,
      };

      switch (visibilityOption) {
        case 2:
          navigate("/master-data/tempat-sambung/kelompok/detail", {
  ...
  const columns: Column<any>[] = [
    ...
    {
      key: "data_desa.nama_desa",
      header: "Asal Desa",
      sortable: true,
      render: (item: any) => {
        return <div>{item.data_desa?.nama_desa || "-"}</div>;
      },
    },
    {
      key: "latitude",
  ...
  document.title = BASE_TITLE + "Kelompok Management";

  return (
    <>
      <div className="relative md:h-full">
        {(isRefetchingKelompok || isLoadingDetail) && (
        ...
        <div className="flex flex-col gap-5 h-full">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-sm mt-0.5">
                      Kelola dan pantau data kelompok dengan mudah
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onKeyDown={(e: any) =>
                    e.key === "Enter" && refetchListKelompok()
                  }
                />
              </div>
              ...
              <div className="flex flex-col md:flex-row justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <button
                    ...
                    onClick={() =>
                      navigate("/master-data/tempat-sambung/kelompok/create", {
                        state: {
                          fetchdataDesa: fetchDataDesa,
                        },
                        replace: true,
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="mt-3 shrink-0">
              <Pagination
                currentPage={dataListKelompok?.meta?.current_page || 1}
                lastPage={dataListKelompok?.meta?.last_page || 1}
                totalItems={dataListKelompok?.meta?.total || 0}
                rowsPerPage={rows}
                onPageChange={(params) => {
