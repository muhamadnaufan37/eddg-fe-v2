...
const UpdateDesa = () => {
  ...
  const validationSchema = () => {
    let validation = Yup.object().shape({
      nama_kelompok: Yup.string()
        .max(255, "Nama desa Max. 255 karakter")
  ...
  const initialValues = {
    ...
    desa_id: dataBalikan?.detailData?.data_desa?.id ?? "",
    nama_kelompok: dataBalikan?.detailData?.nama_kelompok ?? "",
    alamat: dataBalikan?.detailData?.alamat ?? "",
    latitude: dataBalikan?.detailData?.latitude ?? "",
    longitude: dataBalikan?.detailData?.longitude ?? "",
  ...
  const handleSubmit = async (values: any, { setSubmitting }: any) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setTimeout(() => {
          navigate("/master-data/tempat-sambung/kelompok", { replace: true });
        }, 900);
      } else {
          ...
          <Form>
            ...
            >
              ...
              <div className="flex flex-col gap-3 p-4 sm:p-6">
                ...
                <div className="flex flex-col gap-2">
                  <label className={THEME_COLORS.text.label}>Desa Asal</label>
                  <Select
                    id="desa_id"
                    name="desa_id"
                    value={
                      dataBalikan?.fetchdataDesa?.find(
                        (opt: any) => opt.value === values.desa_id,
                      ) || null
                    }
                    options={dataBalikan?.fetchdataDesa || []}
                    onChange={async (option: any) => {
                      setFieldValue("desa_id", option?.value || "");
                    }}
